...
const EditTransactionForm = ({ transaction, onTransactionUpdated, onClose, setCreateError }) => {
  ...
  useEffect(() => {
    if (transaction) {
      ...
      setBorrowStatus(transaction.borrowStatus);
    }
  }, [transaction]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try{
      const updateTransaction = await editTransactionById(transaction.id, userId, bookId, startDate, dueDate, status, borrowStatus);
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Update Transaction</button>
    </form>
    </>
  );
};
