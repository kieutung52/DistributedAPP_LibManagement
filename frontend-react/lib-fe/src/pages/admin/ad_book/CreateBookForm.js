...
const CreateBookForm = ({ onBookCreated, onClose, setCreateError }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const newBook = await createBook(isbn, title, author, publisher, year, quantity);
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text" id="isbn" value={isbn} onChange={(e) => setIsbn(e.target.value)}
        ...
        <div>
          ...
          <input
            type="text" id="publisher" value={publisher} onChange={(e) => setPublisher(e.target.value)}
            className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
          />
        </div>
        <div>
          ...
          <input
            type="number" id="year" value={year} onChange={(e) => setYear(e.target.value)}
            className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
          />
        </div>
        <div>
